...
var sum = function(array) {
  ...
  return (array.length === 0) ? 0 : +copy.splice(0, 1) + sum(copy)
};
...
var sumBelow = function(n) {
  if (n === 0) return 0;
  return (n < 1) ? n+1 + sumBelow(n+1) : n-1 + sumBelow(n-1);
};
...
var exponent = function(base, exp) {
  ...
  return (exp > 0) ? base * exponent(base, exp - 1): 1/base * exponent(base, exp+1);
};
...
var fizzBuzz = function(n) {
  let arr = [];
  if (n === 0) return arr;
  if (n % 15 === 0) arr.unshift('FizzBuzz');
  else if (n % 3 === 0) arr.unshift('Fizz');
  else if (n % 5 === 0) arr.unshift('Buzz');
  else arr.unshift(n.toString());
  return n === 0 ? arr : arr.concat(fizzBuzz(n-1));
};
